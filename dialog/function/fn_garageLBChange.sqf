...
/*
    ...
    Description:
    Kann nicht gestört werden, um darauf zu antworten. Bereits aus Versehen gelöscht...
*/
...
// Überprüfen, ob die Klasse existiert, andernfalls die Standardklasse verwenden
if (!isClass (missionConfigFile >> "LifeCfgVehicles" >> _classNameLife)) then {
    _classNameLife = "Default";
    diag_log format ["%1: LifeCfgVehicles class doesn't exist", _className];
};

// Farboptionen abrufen und auswählen
...
// Fahrzeuginformationen und Trunk-Kapazität abrufen
_vehicleInfo = [_className] call life_fnc_fetchVehInfo;
_trunkSpace = [_className] call life_fnc_vehicleWeightCfg;

// Fahrzeugpreis und Lagergebühr berechnen
...
// Verkaufs- und Abholpreise festlegen, falls ungültig
if (!(_sellPrice isEqualType 0) || _sellPrice < 1) then {_sellPrice = 500;};
if (!(_retrievePrice isEqualType 0) || _retrievePrice < 1) then {_retrievePrice = 500;};

// Informationen in der GUI aktualisieren
