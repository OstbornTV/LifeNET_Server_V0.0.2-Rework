...
if (_exit > 0) exitWith { // exit if variable greater than 0, there is already an entry in the database -> call query funtion
    [_pid,_sender,_side] call cat_craftingV2_fnc_query;
};
...
_queryResult = [_query,2] call DB_fnc_asyncCall; // execute query
...
[_pid,_sender,_side] call cat_craftingV2_fnc_query; // call query function