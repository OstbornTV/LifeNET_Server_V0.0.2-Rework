...
private _queryResult = [_query,2] call DB_fnc_asyncCall; // execute query
...
if (_queryResult isEqualTo []) exitWith { // if result is empty
    [_pid,_sender,_side] spawn cat_craftingV2_fnc_add; // try to add database entry
};
...
private _plans = [_queryResult select 0] call DB_fnc_mresToArray; // parse plans array of index 0
...
for "_i" from 0 to (count _plans)-1 do { // for each plan array entry
    ...
    _plans set[_i,[_data select 0, ([_data select 1,1] call DB_fnc_bool)]]; // convert int to boolean
};
