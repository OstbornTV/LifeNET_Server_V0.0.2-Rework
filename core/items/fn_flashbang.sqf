...
// Parametrisiere die Position des Blitzgranaten-Effekts
private ["_pos", "_eyeDir", "_pPos", "_impactPos", "_norm", "_angle", "_blurEffect", "_ppEffect"];
_pos = [_this, 0, [], [[]]] call BIS_fnc_param;

// Überprüfe, ob die Position gültig ist und der Spieler sich in der Nähe befindet
if (_pos isEqualTo []) exitWith {}; // Ungültige Position
if (player distance _pos > 25) exitWith {}; // Spieler zu weit entfernt

// Korrigiere die Position, um mögliche Kollisionen mit dem Spieler zu vermeiden
_pos set [2, (_pos select 2) + 0.2];
if (lineIntersects [eyePos player, ATLtoASL _pos, player]) exitWith {}; // Kollision mit Hindernis

// Berechne den Winkel zwischen Spielerblickrichtung und Aufprallrichtung
...
if (_angle < 90) then {
    ...
    // Verändere den Farbfilter für dramatischen Effekt
    ...
    // Deaktiviere den Weißbildschirm-Effekt
    _ppEffect ppEffectEnable false;
    ppEffectDestroy _ppEffect;

    // Deaktiviere den Blur-Effekt
    ...
    // Verändere den Blur-Effekt für dramatischen Effekt
