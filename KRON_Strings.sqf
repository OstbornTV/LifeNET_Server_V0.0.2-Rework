...
//                        _titleer=[_str] call KRON_StrUpper
//
...
KRON_StrMid = {
    ...
    _arr=[_in] call KRON_StrToArray;
    _len=count(_arr);
    if ((count _this)>2) then {_len=(param [2])};
    _out="";
...
KRON_StrInStr = {
    private ["_out"];
    _in=param [0];
    _out=if (([param [0],param [1]] call KRON_StrIndex)==-1) then {false} else {true};
     _out
};
...
KRON_ArraySort = {
    ...
    if ((_a select 0) isEqualType []) then {
        _s=0;
        if (((count _this)>1) && ((param [1]) isEqualType 0)) then {
            _s=param [1];
        };
    };
...
KRON_getArg = {
    ...
    _out="";
    if ((count _this)>2) then {_out=param [2]};
    _arr=[_in] call KRON_ArrayToUpper;
