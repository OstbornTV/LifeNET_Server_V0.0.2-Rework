...
if (_uid isEqualTo "" || {_type isEqualTo ""} || {_name isEqualTo ""}) exitWith {}; // Überprüfen, ob ungültige Daten übergeben wurden.
...
if (count _type isEqualTo 0) exitWith {}; // Es handelt sich nicht um unsere übergebenen Informationen...
// Wird ein benutzerdefiniertes Kopfgeld übermittelt? Das als Preis festlegen.
if !(_customBounty isEqualTo -1) then {_type set [1, _customBounty];};
// Suche die Fahndungsliste, um sicherzustellen, dass die Einheit nicht darauf steht.
